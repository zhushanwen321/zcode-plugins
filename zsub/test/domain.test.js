 ...
 *
 * 隔离原则：所有落盘走临时目录（mkdtemp），record/output 经 ZSUB_ROOT 指向
...
/** 每个测试独立 ZSUB_ROOT 临时目录，结束后恢复 env 并清理。 */
function setupRoot(t) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'zsub-domain-'));
  const prev = process.env.ZSUB_ROOT;
  process.env.ZSUB_ROOT = root;
  t.after(() => {
    if (prev === undefined) delete process.env.ZSUB_ROOT;
    else process.env.ZSUB_ROOT = prev;
