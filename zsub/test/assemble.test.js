...
/**
 * assemble.js probe 门控测试（决策位①下沉）：ZSUB_RUNNER=appserver 的
...
after(() => {
  delete process.env.ZSUB_RUNNER;
  try { fs.rmSync(TMP, { recursive: true, force: true }); } catch { /* 尽力清理 */ }
});

test('ZSUB_RUNNER 缺省 → spawn，无探针开销', async () => {
  delete process.env.ZSUB_RUNNER;
...
test('ZSUB_RUNNER=appserver 且 probe OK（fake app-server）→ 保持 appserver', async () => {
  process.env.ZSUB_RUNNER = 'appserver';
  ...
  try {
    ...
    delete process.env.ZSUB_ZCODE_CLI;
  }
});

test('ZSUB_RUNNER=appserver 且 probe 失败（CLI 不可用）→ 降级 spawn，不抛错', async () => {
  process.env.ZSUB_RUNNER = 'appserver';
  process.env.ZSUB_ZCODE_CLI = path.join(TMP, 'no-such-cli.cjs');
  try {
    const a = await assembleManager();
...
test('显式 opts.runner 注入 → 不探测，注入即接管', async () => {
  process.env.ZSUB_RUNNER = 'appserver';
  process.env.ZSUB_ZCODE_CLI = path.join(TMP, 'no-such-cli.cjs'); // 即使探针必败也不应触发
  try {
    const fakeRunner = {
