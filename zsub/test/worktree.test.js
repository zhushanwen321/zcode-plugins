 ...
 * commit 建仓，不 mock git 行为。ZSUB_ROOT 指临时目录，绝不碰真实 ~/.zcode。
 */
...
test('prepare：创建 worktree 目录与 zsub/<id> 分支', async () => {
  ...
  assert.equal(path.dirname(dir), process.env.ZSUB_ROOT, '目录必须落 <zsubRoot>/wt-<id>');
...
test('prepare：主树 dirty 抛可操作错误（文案含恢复指引）且不留半成品', async () => {
  ...
  assert.ok(!fs.existsSync(path.join(process.env.ZSUB_ROOT, `wt-${id}`)));
...
test('sweepStaleOutputs：报告孤儿结果文件但绝不删除（用户资产边界）', async () => {
  ...
  const saved = process.env.ZSUB_ROOT;
  process.env.ZSUB_ROOT = path.join(TMP, 'no-such-root');
