 ...
 *
 * 形态：runner 持一个 `zcode app-server` 长驻子进程（隔离 HOME + ZSUB_NESTED=1，
...
/** 每次 spawn 前重读 env：ZSUB_ZCODE_CLI 的测试覆盖与运行时更换 CLI 都能生效。 */
...
class AppServerConnection {
  ...
  _start() {
    ...
    const child = spawn(this._nodeCmd, [this._cliPath, 'app-server', '--cwd', this._cwd], {
      ...
      env: { ...process.env, ...this._extraEnv, HOME: this._homeDir, ZSUB_NESTED: '1' },
