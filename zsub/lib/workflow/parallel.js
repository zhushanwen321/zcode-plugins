...
async function runParallel({
  task, perspectives, workdir, model,
  ...
  const analyzeEntries = await runWithLimit(ps, maxConcurrent, async (p, i) => {
    ...
    completed++;
    if (onPhase) onPhase({ phase: `analyze:${p}`, status: entry.ok ? `done (${completed}/${ps.length})` : 'failed' });
    return entry;
  });
  ...
  if (failed.length === ps.length) {
    return {
      ok: false, workflow: 'parallel', task, workdir, model: modelRef, phases: phaseResults,
  ...
  if (onPhase) onPhase({ phase: 'aggregate', status: agg.ok ? 'done' : 'failed' });

  const ok = agg.ok;
  return {
    ok, workflow: 'parallel', task, workdir, model: modelRef, phases: phaseResults,
