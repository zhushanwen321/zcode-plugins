 ...
 * 3. 防递归标记从 DWF_NESTED 改为 ZSUB_NESTED=1（DESIGN-v3 D10）。
...
/** 每次 spawn 时重读 env：ZSUB_ZCODE_CLI 的测试覆盖与运行时更换 CLI 都能生效。 */
function resolveZcodeCli() {
  return process.env.ZSUB_ZCODE_CLI || config.ZCODE_CLI;
}
  ...
  const run = new Promise((resolve) => {
    ...
    try {
      childRef = spawn('node', [cli, ...args], {
        ...
        // ZSUB_NESTED 是防递归硬性注入（D10 第一重门禁）
        env: { ...process.env, ...extraEnv, HOME: home, ZSUB_NESTED: '1' },
        stdio: ['ignore', 'pipe', 'pipe'],
      });
    } catch (err) {
      finish({
        status: 'error',
        error: `无法启动 zcode CLI（${cli}）: ${err.message}。恢复指引：用 ZSUB_ZCODE_CLI 环境变量指向有效 CLI 路径后重试。`,
      });
      return;
    }
    ...
    childRef.on('error', (err) => {
      finish({
        status: 'error',
        error: `无法运行 zcode CLI（${cli}）: ${err.message}。恢复指引：确认 CLI 存在可执行，或用 ZSUB_ZCODE_CLI 指定路径。`,
      });
    });
