 ...
 *
 * 目录约定（真实 HOME 下）：
 *   ~/.zcode/zsub/                 插件数据根（ZSUB_ROOT 可覆盖，测试用）
 ...
 *   ZSUB_NESTED=1           防递归标记（D10）：本进程若在嵌套环境直接拒绝服务
 */
...
const ZCODE_CLI = process.env.ZSUB_ZCODE_CLI || '/Applications/ZCode.app/Contents/Resources/glm/zcode.cjs';
const V2_CONFIG_PATH = path.join(os.homedir(), '.zcode', 'v2', 'config.json');

/** 插件数据根。测试可用 ZSUB_ROOT 指到临时目录。 */
function zsubRoot() {
  return process.env.ZSUB_ROOT || path.join(os.homedir(), '.zcode', 'zsub');
}
...
const NESTED = process.env.ZSUB_NESTED === '1';
