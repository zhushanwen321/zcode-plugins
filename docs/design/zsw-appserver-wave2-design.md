...
- **采用**：端口加方法 `release(exec)`：apc 实现 = `session/close`（控制面 1.5s 超时，best-effort）+ `_sessions` 注销（chunks 缓冲随条目删除回收）；`exec.sessionId` 未回填（create 前失败/取消的早期终态）时直接 no-op。spawn 实现 = no-op（进程已退出，无对应物）。**`wrapWithProbeInvalidation` 包装层必须同步转发 `release`**（按 `exec.kind` 路由到 inner/spawnRunner，与 resume/alive 同款）——fromCache 路径下 run-phase 拿到的是包装 runner，漏转发则 `runner.release?.(exec)`（run-phase 实际调用形态）静默短路——释放永不发生且无报错，是比 TypeError 更隐蔽的驻留泄漏（转发面由 test/appserver.test.js 契约用例钉住）。调用点：run-phase 在 done 落定后（成功/失败/中止/超时全终态）以 `runner.release?.(exec)` 可选调用（best-effort 语义 + 测试 fake runner 不强制实现）——一次性阶段无续聊语义，会话价值已由 persistence:immediate 落盘保住（可 list 可查可 resume，见 G6 判定）。zsub 线不调 release（conversation 续聊依赖驻留/恢复序，行为不变）。
...
- **安全边界（交付后复审补强）**：「本任务 spawn 重跑」只允许 invalidating 错误发生在 create/probe 阶段（prompt 投递被接受之前）——轮中失败出口（`_failAllTurns` 连接中断的纯 reason、`turn.fail`、分支 B 文案）不携带 `errorKind:'protocol-drift'` 且文案无 `[-326xx]`，不触发重跑，副作用阶段无双执行；若未来轮中错误出口可能携带漂移分类，须先按 D3「副作用不可知，不自动重跑」同款理由重新论证（回归钉：test/assemble.test.js isInvalidatingError 轮中错误形态用例）。
...
> **Gate B 可达性签收注记（2026-08-30）**：zsw 1.0.0 起 MCP tool face offline（zflow/zsub tool 物理不可达），且 workflow run 恒本地前台（无 daemon 化形态）——B-2 daemon 臂、B-3、B-8② 的「zcode 会话内调 zflow」路径在当前产品形态**不可达，Gate B 按 blocked 签收**（等价证据面：B-2 daemon 臂 ≈ 一波 A-8 真机共享连接并发 + workflow-apc fake 用例 + 对照臂真机；B-3 ≈ workflow-apc 用例② + 检查点 4 探针链 + D3 形态边界；B-8② ≈ dispatchToolCall 挂点单测，face 重启即自然生效——workflow 线 runner 注入在 assembleManager 内两入口同源，MCP face 恢复后无需改动）。实际真机签收：B-1/B-2 对照臂/B-4/B-5/B-6（降档 chain 3 阶段，token 纪律）/B-7/B-8①/B-9 全 pass。
...
| 2026-08-30 | 双级验收完成：Gate A 全量 585 tests / 582 pass / 0 fail / 3 skip（既有真机占位）exit 0——首轮 1 fail 为既有并发计数用例的测量面 flaky（pairwise 重叠计数在全量负载下虚高峰值），正面修复为扫描线测量（8c6e2fd，断言未削弱、饱和压力 3 轮零失败）后重跑全绿；Gate B 9 场景真机签收 8 pass / 0 fail / 3 blocked（B-2 daemon 臂/B-3/B-8② 因 MCP tool face offline 物理不可达，§4 表后补可达性签收注记，等价证据面单测+探针链+对照臂覆盖，无 mock 冒充）；B-6 按 token 纪律降档 chain 3 阶段（判据面不变） | dev-flow Gate A subagent 两轮 + Gate B subagent 真机执行（约 22 个真实模型阶段轮） |
| 2026-08-30 | 交付后一致性复审（双 reviewer：W1 线 + W2/W3/验收后修正批次；0 must-fix / 3 suggestion / 3 doc_error，承重断言经主 agent 亲验）：① D2 漏转发失效模式更正——`runner.release?.(exec)` 调用形态下漏转发是静默短路丢释放（驻留泄漏无报错），非 undefined TypeError；② D4 补「本任务 spawn 重跑」安全边界（invalidating 仅 create/probe 阶段可达，轮中错误出口不带漂移分类；回归钉入 assemble.test.js）；③ D5 DRIFT 常量定义行号重锚 :368/:370（修正批次中部插行致漂）。配套代码修正：buildRuntimeModel 闸门外 `providerId\|\|DEFAULT` 兜底行移除（部分登记边界下杂交组合不落显式 throw，与台账声明不符）、闸门注释更正（清单解析 ≠ 凭据存在，凭据由 provider 条目检查兜住）、直测用例①登记值改可区分值 | tech-design-review subagent R1/R2（报告 /tmp/design-review-zsw-wave2-post-R1.md、-R2.md）+ 主 agent 亲验修复 |
