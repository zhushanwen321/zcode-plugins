...
| V7i | 基础设施（设计 V7 后半，⛔B）：slots 收敛 createConcurrencyPool strict-fifo 薄层（导出签名不变；**迁移后 ⛔B 须加深度分层回归断言——core 池授予排队条目时复查 effectiveMaxConcurrent**）、output-store.atomicWrite 与 notifier-mailbox tmp+rename 改调 `writeAtomicFileSync`、prune 接线（daemon 启动/session-start + env 上限透传 + `configureCore({log})` warn 通道确认） | `lib/slots.js`、`lib/output-store.js`、`lib/notifier-mailbox.js`、`lib/assemble.js`（prune 接线点）、daemon 启动文件（首步定位登记）、相关测试 | V4o、V0a | plain | ⛔B slots 现有排队测试在 strict-fifo 全绿 + 分层配额不穿透断言；S6 原子写语义保持（残留 tmp 清理属 core 内部）；workflow-state 收敛断言就绪（真机部分归 V9） |
...
| Unit | 状态 | 轮次 | 证据指针 |
|------|------|------|----------|
| V0a | pending（G-user 已过：清理批 f739ce4/b9ae43f 落库） | - | - |
| V0b | pending（G-user 已过；首步核实简化批是否已消 modelEntries 双份，若已消则本单元降级为等值验证） | - | - |
