...
- 2026-09-01：**R1 修复闭环 + 定向复审判「基本收敛」**：12/13 unreasonable 闭环、doc_errors 抽查全一致、2 requirements gap 实证闭环（CI 门禁红绿双态 + refreshHint 双受众）、无行为破坏、393/393 绿；3 条 minor 尾巴已收口（writePatch 退役 666405a、findWorkspaceRoot 守卫、S1① 例外清单补 hasProviderCredentials）。**阶段 3/4 终态：unreasonable 0 / doc_errors 0 / reasonable 入登记表**。剩余 V8n/V9s blocked（G-npm：core 0.4.0 发 npm，用户侧）——按 D-E2，Gate A 全量与 Gate B 真机场景在 --npm 终态后执行避免双跑。
- 2026-09-01：**交付后复审第二轮（适配性专项）+ 「全部修复」**：四分区只读审查（vendored↔core 等值 / zsw call-site / pi 横向对齐 / 下沉完整性）结论——适配非漂移（114 符号双向零差异、sha 门禁 19/19、bundle 自包含隔离探针实证）、A 侧无新应下沉候选、决策记录交叉核对零冲突；发现 2 must-fix（host.runAndWait 丢弃 norm.model 与 budgetTimeMs：CLI `--model`/`--timeout-ms` 同步面失效 + hang 永久挂起）+ 1 死条件（agent-runner-adapter `!== undefined` 恒真）+ recoverOrphans 计数口径混同 + resolveGitRoot 绕开 core.gitRun + vendor 溯源三缺口。修复跨两仓：B 仓 699a94afc（core `runAndWait` 加第 6 参 model 透传 spec Option B；`recoverCrashedRuns` 返回 `{loaded, recovered}` 计数；pi 回接 recoverCrashedRuns/runSummary 消最后两处双实现；changeset 补 0.3.0 幽灵版本覆盖注记；README D5 加 0.x semver 例外——core 2811/pi 917/typecheck/build:bundle 全绿）；A 仓本 commit（runAndWait 透传 model + budgetTimeMs 作轮询 deadline（三测试钉住，真实 core 链路）；recoverOrphans 改 `{rehydrated, orphaned}` 双口径消费 core 返回值、消宿主预读 loadAll；死条件改判 null；resolveGitRoot 改走 core.gitRun 统一超时/错误包装；vendor 脚本加 sourceRev/sourceDirty 溯源 + 外部依赖子路径正则 + 隔离加载探针裁决自包含 + distMappedFrom 落位映射留痕，vendored 已按新脚本刷新至 B 仓 699a94afc）。维持现状项（有裁决/无害）：slots effectiveLimit 与 buildSourceIndex 两处已声明镜像（core 无导出面）、atomic-write 清扫族零消费导出（major 窗口再评）、0.3.0 幽灵版本本体（无法追发，changeset 注记兜底）。
