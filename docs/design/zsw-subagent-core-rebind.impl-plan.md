...
| U1-vendor | zsw | vendor 基建 + 仓规修订 + 单一解析点 | `<W>/scripts/vendor-subagent-core.js`(新)、`<P>/lib/vendor/subagent-core/`(新，vendored 产物入 git：index.cjs+workflows/+VENDOR-MANIFEST.json)、`<P>/lib/core-ref.js`(新，vendorDir()/workflowAssetPath()/requireCore()/vendorManifest()；V6-② 版本 guard 以 `test/core-ref.test.js` 守卫测试面替代——偏差 #14)、`<W>/AGENTS.md`(零依赖红线例外条款)、`<W>/docs/standards.md`(vendored 消费节)、`<P>/README.md`(消费说明) | U0 | plain | ①干净进程经 core-ref require 成功（runWorkflow/configureCore 为函数）；②check-sync/check-pack 双绿（package.json 仍零 dependencies）；③vendored `workflows/review-fix-loop-utils.cjs` 与 core 源字节一致（vendor 脚本自检断言）；④standards.md 记录三形态（inline/marketplace/npm 包）推演 |
...
| 9 | `findIssueKey` 编排层消毒包装（`Object.hasOwn` 首行），MF-1 护栏改测包装出口 | 分叉点⑥（2026-08-30 zsw 侧 MF-1 修复，core 资产为 pi 原版 truthy 查表） | vendored 资产禁改 + 护栏不削；上游修复（core workflows 资产 findIssueKey + 主脚本自查）已记入 U0 批次，落地后包装变恒等、随上游对齐拆除（登记「待上游对齐」）——本条即设计失败模式 B（行为不一致各自修）的活案例与根治路径。**已闭环（2026-08-30）**：上游 88d7eadc6 已修复（findIssueKey Object.hasOwn 加固入 core 资产），zsw 侧消毒包装随 U3 切 vendored 资产消亡——「待上游对齐」拆除条件兑现 |
...
| 13 | reaper（`lib/reaper.js` 启动期孤儿 worktree 回收 + outputs 陈旧报告）保留 | 设计 D6-⑦「run 生命周期资产回收归 core」语境 | reaper 回收的是 outputs/ 与 worktree 壳侧资产（core 无对应面——outputs 是用户资产只报告不删，worktree 是 zsub 自建派生资源）；run 崩溃恢复已归 core recoverOrphans（FileRunStore 孤儿终态化），两者职责不重叠 |
| 14 | V6-② 启动版本 guard（`CORE_VENDOR_VERSION` 比对 + 报错恢复指引）不实现，以 core-ref 守卫测试面替代 | 设计 D5「^ 区间依赖 + 启动期 guard」配套（偏差 #1 改 vendored 后 guard 失去比对对象） | vendored 形态无版本错配物理面（副本由 vendor 脚本 + manifest sha256 锁定，不存在运行时解析到别版本）；守卫转测试面：`test/core-ref.test.js` 断言 selfContainedIndex 形态 + 2b/2c 消费符号在场 + 错误路径 require 失败报错含恢复指引（refreshHint 按 manifest source 分流 `--local`/`--npm`）兜底 |
