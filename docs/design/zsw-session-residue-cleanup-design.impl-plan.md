...
| 11 | 停机校验③嵌套标记升级为双标记匹配（isNestedCommand + 拒绝文案 + 文档面同步）；检出面如实声明为 argv 字样形态、env 段可见性不可靠（2026-09-06 复核） | ③ 对真实嵌套形态本检不出，靠①②④兜底；双标记能捞则捞，与 config.js isNestedEnv 口径对齐 | 已固化（design-code-sync R1/R2） |
