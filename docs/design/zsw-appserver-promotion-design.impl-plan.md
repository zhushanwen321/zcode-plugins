...
| 2026-08-29 | R2 批次 | probe() 探针会话 create 不带 persistence:'immediate'（任务会话才固定携带） | probe 会话即弃（create→close→shutdown），deferred 默认恰好不留排障噪音；D4「固定携带」的语义主体是可恢复+list 可见的任务会话面 | 设计 D4 无需改（实现层例外）；已在此登记 |
| 2026-08-29 | R2 批次 | probe 重探 ok 后将 ok 结论写回缓存（D1 只要求「失效并重探一次」） | 写回避免后续每次组装重复付探针成本；不违反「失败不落盘」纪律（写回的是 ok 结论） | 设计 D1 末尾已补一句（本轮 doc 修订顺带） |
...
| 2026-08-29 | **第二轮三区一致性审查（全量重审 225930d..HEAD）**：区 A（apc 引擎核心）0 unreasonable/4 doc_errors、区 B（通道编排标注）3 low unreasonable/2 doc_errors、区 C（测试/MCP/文档发布面）0 unreasonable/12 doc_errors——实现与设计一致，主要问题为 Gate B D3 观测面修正的回填不彻底（G4/A-5/README/CONTEXT/代码注释仍留「stderr=thinking 观测面」旧定性）；修复批次 R2：3 low + 5 注释项 subagent 落地（tmp 并发踩踏、降级标注端到端断言、降级出声断言、A5/A-8/观测面过时注释、model-router/ports JSDoc 漂移，测试 101/101+46/46 绿），doc_errors 主 agent 修订；合理演化 8 条登记 §5；tag re-tag 1.2.0（仍从未 push 曝光） | 用户指示的第二轮对抗式审查 |
