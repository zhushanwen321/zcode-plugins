...
[zsw hooks/hooks.json] ──spawn──► node bin/zsw-hook.js（薄入口；ZSW_NESTED=1 ? → 输出 {} 退出）
   ...
   │                                  │    标记解析与 zsw models 同源走 defaultModelRef）
