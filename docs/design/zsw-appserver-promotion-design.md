...
| G4 | thinking 可调：`zsw start --thinking <level>` 生效且非法值容错 | 引擎自有日志 jsonl 中 thoughtLevel / thought_level_skipped 事件命中（Gate B 实证观测面，见 D3 修正）；zsw logs/ 落盘为异常诊断面；非法值 warn 跳过不失败 |
...
- **采用**：**两个入口来源**，均落 `create.toolAllowlist/toolDenylist`——① CLI flag：`zsw start --allow-tools <逗号分隔清单>` / `--deny-tools <逗号分隔清单>`（分隔符与 spawn 通道 `--disallowed-tools` 一致）；② agent .md frontmatter：`taskCtx.disallowedTools`（设计时点 `lib/manager.js:202-206` 组装，实施后行号已漂移；现状 spawn 通道经 `lib/driver.js:242` 落 `--disallowed-tools` 引擎级硬拦截，而 appserver runner 现状不消费该字段——grep `disallowed|toolDenylist` 零命中）同样映射 `create.toolDenylist`，与 CLI flag 取并集去重。**frontmatter `tools` 白名单显式决策：维持 buildPrompt 软约束**（`lib/prompt-builder.js:79-95`），不升级为 `toolAllowlist`——对齐 spawn 通道现状语义（白名单一直无 flag 通道、仅软约束，`lib/manager.js:202-203` 注释），且白名单硬约束的误杀面（模型无法用清单外工具补救任务）大于黑名单；软约束段黑名单文案「平台层亦会拦截」（`lib/prompt-builder.js:91`）在来源②接线后两通道均恢复为真。现有 spawn 通道 denylist 经 `--disallowed-tools` 的路径保留为回退位行为。
...
| A-5 | thinking | `--thinking low` 跑 run，另跑 `--thinking ultra` | 前者引擎自有日志（`~/.zcode/zsw/home-appserver/.zcode/cli/log/<date>.jsonl`，D3 Gate B 修正后的权威取证面）含 `thoughtLevel:"low"` 事件；后者 `thought_level_skipped` 事件、任务不失败（引擎容错，P2 语义，真机已签收）；不带 flag 的 run 日志含默认档事件（GLM 默认 max 沉淀）；`~/.zcode/zsw/logs/` 落盘为异常诊断面，正常任务零输出 | G4 |
| A-6 | 工具限制 + 遥测 | 三来源各验一条：CLI `--deny-tools "Bash"`（裸工具名，格式保底形态）任务里 prompt 诱导跑 git；**agent .md frontmatter `disallowedTools` 声明的工具**（D6 来源②；翻转前该来源在 spawn 下已是引擎级硬拦截，验翻转后等价）同任务诱导；spec 形态（`Bash(git *)`）按实测结论追加（Gate B A-6 未验 spec 形态，转残留长线观察，见实施计划残留风险 2）；随后检查 `~/.zcode/zsw/home-appserver/` 无遥测标识文件 | 工具被拒（引擎层拦截，非 prompt 软约束，两来源同标准）；`--allow-tools` 白名单同理；遥测目录检查为 D8 验收 | G5 |
...
2. 工具限制格式契约：裸工具名确定合法（源码证据）；括号 spec 形态（`Bash(git *)`）是否被引擎解析、以及 denylist 引擎级拦截的行为面验证——**均归 A-6 真机验收收口**（裁决：不为此增加冒烟 token 成本——冒烟是升级后高频操作保持极小，行为面验证属一次性验收场景；E9 冒烟仅保留参数面无漂移断言）。**Gate B 结果（2026-08-29）**：A-6 已签收两来源引擎级拦截（rollout 内 tools 清单核验）；spec 形态未验，转残留长线观察。
...
| 2026-08-29 | 第二轮一致性审查修订：G4 判定标准与 A-5 通过标准观测面改引擎自有日志 jsonl（兑现 D3 Gate B 修正的回填遗漏——首轮只改了 D3 正文，G4/A-5 行漏改）；D3 冒烟断言面改记 toolDenylist 参数面（行为面归 A-6，与检查点 2 裁决对齐）；A-6 spec 形态改记长线观察 + 检查点 2 补 Gate B 结果标注；D1/D6 证据行号补「设计时点」标注（对齐 D2/D3 先例）。实现侧零 unreasonable，3 条 low（降级标注端到端断言、降级出声断言、probe tmp 并发踩踏）与 5 处过时注释已随修复批次落地 | 第二轮三区对抗式一致性审查（区 A/区 B/区 C 独立报告） |
