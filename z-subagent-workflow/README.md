...
- **`zsw doctor clean`（执行）**：**必须完全退出 ZCode（含菜单栏常驻）后的停机窗口执行**——运行中清理会与 GUI 内存态/同步器/引擎库写入冲突。执行自动完成：四项停机校验（GUI 进程 / app-server 进程 / ZSW_NESTED 子进程 / 双库独占开锁，任一不过即拒绝）→ 三段磁盘校验（备份/删除/VACUUM 三阶段空间门槛，不足时给 `--fs-only` 腾空间指引）→ 双库三件套快照备份（`*.sqlite` + `-wal` + `-shm`，keep-1）→ 引擎库分块删除（≤200 会话/事务 + 批间 checkpoint，13 张引用表 FK 列级联动）+ VACUUM 回收空间 → 索引库联动 → 文件面清理。结果逐项报数。
- **`--fs-only`**：只清文件面（artifacts 集内目录 / exec sess_ 前缀空壳 / log 超 14 天整文件）。**停机校验不豁免**——artifacts 删除会破坏运行中会话的转录引用、log 删除破坏写入句柄，不停机一样不安全。
