 ...
 * - ZSW_ZCODE_CLI    → 临时 fake CLI（读 --prompt，输出单行 JSON）
...
test('runPhase：CLI 非零退出 → ok:false + error 含退出码', async () => {
  ...
  process.env.ZSW_ZCODE_CLI = path.join(TMP, 'not-exist.cjs'); // node 报 module not found，退出码 1
  try {
...
function useMarkerCli() {
  ...
  process.env.ZSW_ZCODE_CLI = MARKER_CLI;
