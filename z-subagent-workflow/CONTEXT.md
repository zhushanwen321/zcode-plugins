...
├── engines/zcode/home-<prov>-<m>/ core zcode 引擎数据（回接 2c：per-provider+model 隔离
│                                 HOME 池 + journal，布局归 core paths SSOT，目录名路径段
│                                 编码——模型名中的 '.' 归一为 '-'）
├── wt-<subagentId>/              worktree 隔离目录（listOrphans 按前缀认领）
└── home-*/                       旧 spawn/appserver 池目录（已随 2c 退役，存量残留可清理）
```

## 执行通道（回接 2c：appserver 退役，统一 core zcode engine）

执行链 = vendored `@zhushanwen/subagent-core` 的 zcode 引擎 spawn 单轮
（`lib/runner-core.js` 端口适配；per-provider+model HOME 池 / 凭据预检 /
兜底模型归引擎 preparer，stdout 解析归 core parser）。appserver 常驻通道按
设计 D6-⑥ 显式退役（回归路线 P3，见 README「回接 2c break 变更」节）。

| env | 取值 | 默认 | 语义 |
|-----|------|------|------|
| `ZSW_RUNNER`（已退役） | `spawn` \| 其他 | 未设置 | 'spawn' = 兼容 no-op（告警一次后忽略——通道本就是 spawn 单轮）；'appserver' = 启动即报退役错误（信息含 D6-⑥ 与 P3 指引）；其余未知值 = 前置报错 |

> CLI 路径覆盖从 `ZSW_ZCODE_CLI` 改为 core 引擎的 `XYZ_ZCODE_CLI`（ Vendored
> 面约定）；探测/漂移检测归 core 引擎探针（binary + version + golden 干跑）。
