...
| 2026-09 · vendored 0.5.1→0.6.0（turn 两计时器 + 误杀修复） | zcode turn 超时从旧固定 300s 墙钟（实测误杀 21% 活跃任务）改两计时器：idle 主判 30min（事件流刷新，活跃零误杀）+ 总上界 60min（固定，env `XYZ_ZCODE_TURN_*` 可调可关）；超时错误面前缀从 `engine_run_failed` 变 `engine_timeout`（turn 超时专报，排障勿再 grep 旧 token）。settled watchdog 两段重锚（收尾段 600s + 中段无进展 30min，env `XYZ_SUBAGENT_SETTLED_WATCHDOG_MS`）；timeout abort 链三态裁决/瞬态重试/dispose 收割随入。0.6.0 附 sync-collect v2 契约（pi 宿主工具面，zsw CLI 不消费）与引擎 dispose 修复；vendor 通道自 0.6.0 恢复 `--npm`（0.4.0/0.5.1 tarball 缺 dist.bundle 期间只能 `--local`） | README「排障 · 超时行为与调优」节、docs/standards.md「vendored 核心包消费」节 |
