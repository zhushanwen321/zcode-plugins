  ...
  return {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        thinking: { type: 'string', description: 'start 可选。thinking 档位（如 low|high|max，合法值按模型动态，GLM 默认 max）。appserver 通道映射 create.thoughtLevel，非法档位 warn 跳过不失败；spawn 回退通道不可用并如实标注' },
        allowTools: { type: 'array', items: { type: 'string' }, description: 'start 可选。允许工具名清单（裸工具名）。MCP 直调面传字符串数组，如 ["Read","Grep"]；CLI 面为逗号分隔字符串（--allow-tools "Read,Grep"）。appserver 通道落 create.toolAllowlist 引擎级白名单；spawn 通道不消费' },
        denyTools: { type: 'array', items: { type: 'string' }, description: 'start 可选。禁止工具名清单（裸工具名）。MCP 直调面传字符串数组，如 ["Bash","WebSearch"]；CLI 面为逗号分隔字符串（--deny-tools "Bash,WebSearch"）。appserver 通道落 create.toolDenylist 引擎级拦截，与 agent .md frontmatter disallowedTools 并集去重；spawn 通道不消费 CLI 来源（frontmatter 来源走 --disallowed-tools 硬拦截）' },
...
/**
 * zflow tool 定义（N2-b 管理面：run 后台化 + abort/status/list/
