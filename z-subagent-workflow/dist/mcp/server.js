  ...
  return {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        thinking: { type: 'string', description: 'start 可选。thinking 档位（如 low|high|max，合法值按模型动态，GLM 默认 max）。spawn 单轮通道不消费该请求值——请求后终态 record 落 thinking="null (spawn 降级)" 如实标注' },
        allowTools: { type: 'array', items: { type: 'string' }, description: 'start 可选。允许工具名清单（裸工具名）。MCP 直调面传字符串数组，如 ["Read","Grep"]；CLI 面为逗号分隔字符串（--allow-tools "Read,Grep"）。spawn 单轮通道无白名单 flag 通道不消费——终态 record 落 toolsNote 如实标注' },
