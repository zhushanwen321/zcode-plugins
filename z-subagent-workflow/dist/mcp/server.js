  ...
  return {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        'allow-tools': { type: 'string', description: 'start 可选。逗号分隔的允许工具名清单（裸工具名，如 "Read,Grep"，语义同 CLI --allow-tools）。appserver 通道落 create.toolAllowlist 引擎级白名单；spawn 通道不消费' },
        'deny-tools': { type: 'string', description: 'start 可选。逗号分隔的禁止工具名清单（裸工具名，如 "Bash,WebSearch"，语义同 CLI --deny-tools）。appserver 通道落 create.toolDenylist 引擎级拦截，与 agent .md frontmatter disallowedTools 并集去重；spawn 通道不消费 CLI 来源（frontmatter 来源走 --disallowed-tools 硬拦截）' },
