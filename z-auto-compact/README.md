...
agent 自助面：bin/zac.js usage（CLI 直查同一只读库）+ skills/zac-usage（解读与压缩引导）
档位状态：~/.zcode/z-auto-compact/state/<sessionId>.json（fired 去重，与引擎零共享）
```
...
1. `~/.zcode/cli/config.json` 的 `plugins.dirs` 数组追加本插件目录绝对路径（`<repo>/z-auto-compact`）。
2. 重启 ZCode（GUI 只在启动时扫描插件配置，改完必须重启才生效）。
3. 验证：终端跑 `zcode plugins list` 应见 `z-auto-compact` enabled。
...
```bash
node bin/zac.js usage [--session <sessionId>]
```

- 默认 `--latest`：按 cwd 精确匹配反查当前项目最近活跃主会话（agent Bash 环境无 sessionId 环境变量，在项目目录下直接跑默认即可）。
- `--session <sessionId>` 显式指定会话（id 见提醒文案或会话 UI）。
- stdout 输出单行 JSON：`{"sessionId":...,"contextTokens":...,"firedTiers":[...],"nextTier":...,"note":"..."}`。字段解读与压缩建议姿势见 skill `zac-usage`。
...
`~/.zcode/z-auto-compact/config.json`（可选，无文件用默认；坏 JSON 回退默认并记 warn；mtime+size 缓存热加载，改完下一轮生效）：
...
- **turn 内实时预警不做**：检测点 = 用户提交 prompt 的时刻，单次长 turn 中途无检查点，由 agent 按 zac-usage skill 主动自查补盲。
